...
public class ErrorLoggingServiceEsImpl implements ErrorLoggingService {
    ...
    public void logAndPersistError(ErrorInfo errorInfo) throws Exception {
            ...
            errorInfo.setCreateTime(LocalDateTime.now().toString());
            IndexResponse response = esDocumentService.createByJson("errors", UUID.randomUUID().toString(), JSON.toJSONString(errorInfo));
